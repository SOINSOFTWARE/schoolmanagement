 ...
 * @since 20/10/2015
 */
public class UserTypeXAccessDAO extends AbstractDAO implements
		IDataAccesable<Cnusertypexaccess> {
	...
	public Set<Cnusertypexaccess> select() {
		...
		try {
			...
			this.stopChronometerAndLogMessage(chrono,
					UserTypeXAccessDAO.class.getName() + ", Select function");
		}
	...
	public Cnusertypexaccess selectByIdentifier(Integer idUserType,
			Integer idAccess) {
		...
		try {
			...
			session.disconnect();
			this.stopChronometerAndLogMessage(chrono,
					UserTypeXAccessDAO.class.getName()
	...
	public void save(Cnusertypexaccess record) {
		...
		try {
			...
			Cnusertypexaccess cnusertypexaccess = this.selectByIdentifier(
					cnusertypexaccessId.getIdUserType(),
					cnusertypexaccessId.getIdAccess());
