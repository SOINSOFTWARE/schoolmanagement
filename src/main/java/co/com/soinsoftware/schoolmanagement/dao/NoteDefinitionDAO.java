 ...
 * @since 15/10/2015
 */
public class NoteDefinitionDAO extends AbstractDAO implements
		IDataAccesable<Bznotedefinition> {
	...
	public Set<Bznotedefinition> select() {
		...
		try {
			...
			this.stopChronometerAndLogMessage(chrono,
					NoteDefinitionDAO.class.getName() + ", Select function");
		}
	...
	public Bznotedefinition selectByIdentifier(Integer identifier) {
		...
		try {
			...
			session.disconnect();
			this.stopChronometerAndLogMessage(chrono,
					NoteDefinitionDAO.class.getName()
	...
	public void save(Bznotedefinition record) {
		...
		try {
			...
			chrono.stop();
			this.stopChronometerAndLogMessage(chrono,
					NoteDefinitionDAO.class.getName() + ", save function");
		}
	}
